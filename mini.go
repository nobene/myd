...
type Editor struct {
	...
	// internal clipboard coordinates
	startx, starty int
	stopx, stopy int

	clip string //internal clipboard
...
//	var clip string
	...
	case key(ctrl('a')):
		e.SetStatusMessage("Selection starts at " + strconv.Itoa(e.cy + 1) + ":" + strconv.Itoa(e.cx + 1))
		e.startx = e.cx
		e.starty = e.cy
	case key(ctrl('c')):
		if e.filename == "" {
...
//				cx3--
	...
	case key(ctrl('q')):
		...
		if e.dirty > 0 && e.quitCounter < quitTimes {
			e.SetStatusMessage(
				"File has unsaved changes. Press Ctrl-Q %d more times to quit.", quitTimes-e.quitCounter)
			e.quitCounter++
			return nil
		}
		os.Stdout.WriteString("\x1b[2J") // clear the screen
	...
	for y := 0; y < e.screenRows; y++ {
		...
		if filerow >= len(e.rows) {
			if len(e.rows) == 0 && y == e.screenRows/3 {
				welcomeMsg := fmt.Sprintf("Myd editor -- version %s , to quit press Ctrl-Q", version)
...
func main() {
	...
	editor.SetStatusMessage(" ^Q = quit | ^S = save | ^D = del | ^F = find | ^A = mark | ^C = copy | ^V = paste | ^X = cut | ^Z = clear")
